...
export const T = {
  ...
  // 실시간 데이터 지연 고지
  delayTitle: '⏱ 실시간 데이터 지연 안내',
  delayBody: 'Cloudflare 등 실측 지표는 제공처가 측정값을 집계·발표하는 데 시차가 있어, 그래프의 최신 지점이 현재보다 뒤처질 수 있습니다(지연 폭은 지표·제공처마다 다름). 존재하지 않는 시점의 값을 임의로 만들지 않기 위함이며, 데이터가 갱신되면 자동 반영됩니다. (시뮬 지표에는 해당하지 않습니다.)',
  // 데이터 소스 설정 모달
  apiButton: '데이터 소스',
  apiTitle: '데이터 소스 설정',
