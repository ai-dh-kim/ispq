...
// NIA 속도측정 지표 공통 해석 주석(8종 동일). 문구는 2026-07-15 사용자 지정 그대로.
const NIA_NOTE =
  '※ NIA가 직접 운영하는 측정 시스템(speed.nia.or.kr)의 사업자별 속도측정 데이터 입니다. 이용자가 자발적으로 측정한 표본(가입 상품은 이용자 신고값)이라 절대값보다 사업자 간 상대·추세 비교에 적합합니다. 국내 전용이라 해외 ISP 비교는 없는 대신, 케이블 사업자(딜라이브·LG헬로비전·HCN·CMB)까지 비교할 수 있습니다. 일부 ISP는 해당 상품의 유효 표본이 없어 지표 변화가 없을 수 있습니다(예: CMB 1G 상품). 원자료는 매일 자정(KST) 한 차례 갱신되는 최근 30일 집계이며, 각 날짜는 그 날까지 반영된 값입니다. RAW데이터 수집 시작 일자(2026-07-15) 이후부터 일별 이력이 쌓입니다.';
