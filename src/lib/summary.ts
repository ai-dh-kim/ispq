...
//
// 시간창은 '지금'이 아니라 지표별 마지막 실데이터 시점(lastMs) 기준:
//   최근 창 = (lastMs-7일, lastMs]  ·  기준 창 = (lastMs-37일, lastMs-7일]
...
export interface SummaryCell {
  v: number | null;    // 최근 7일 평균 (데이터 없으면 null)
...
function windowMeans(
  data: QualityData, ispIds: string[], metricId: string,
  ...
  const recentFrom = lastMs - RECENT_DAYS * DAY;
  const baseFrom = lastMs - (RECENT_DAYS + BASE_DAYS) * DAY;
  for (const isp of ispIds) {
    const v = data.series[isp]?.[metricId]?.coarse?.[0];
...
export function buildSummary(
  data: QualityData, ispIds: string[], metricIds: string[],
  ...
  for (const metricId of metricIds) {
    ...
    const means = windowMeans(data, ispIds, metricId);
    const vals = ispIds
...
// 변화 감지: 최근 7일 vs 이전 30일 평균의 변화율. |변화율| >= minPct 만 반환, 악화 우선 정렬.
export function detectChanges(
  data: QualityData, ispIds: string[], metricIds: string[], minPct = 10,
): ChangeItem[] {
